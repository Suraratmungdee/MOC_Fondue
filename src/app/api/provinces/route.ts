...
export async function GET(req: NextRequest) {
    try {
        ...
        if (sdate && edate && provinceNames.length > 0) {
            const [data] = await pool.query(`SELECT * FROM scraped_data WHERE (res_date >= ? AND res_date <= ?) AND (JSON_EXTRACT(res_data, '$.province') REGEXP ?)`, [sdate, edate, provincePattern]);
